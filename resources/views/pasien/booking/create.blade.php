            ...
            fetch(`/api/dokters/${poliId}`)
                ...
                .then(data => {
                    ...
                    data.forEach(dokter => {
                        ...
                        let jadwalStr = 'Jadwal belum tersedia';
                        if (dokter.jadwals && dokter.jadwals.length > 0) {
                             jadwalStr = dokter.jadwals.map(j => `${j.hari} (${j.jam_mulai.slice(0,5)}-${j.jam_selesai.slice(0,5)})`).join(', ');
                        }
